  ...
  const handleRunAdvancedTest = async () => {
    ...
    try {
      ...
      if (!sourceResult.success) {
        setError(sourceResult.error || "Chyba při načítání testovacího souboru");
        setProcessState(ProcessState.ERROR);
        return;
      }
        ...
        return (
          <Card className="mt-6">
            <CardBody>
              <div className="flex flex-col items-center p-4">
                <Spinner size="lg" color="primary" className="mb-4" />
                <p className="text-lg mb-4">Probíhá pokročilé zpracování dat...</p>
              </div>
            </CardBody>
          </Card>
        );
                ...
                <div className="mt-2 mb-6 border rounded-md p-4">
                  ...
                  <div className="space-y-4">
                    {result.reportData.dataAdded !== undefined && (
                      <p className="text-lg">
                        Počet přidaných záznamů: <strong>{result.reportData.dataAdded}</strong>
                      </p>
                    )}
              ...
              {result?.finalFilePath && (
                <div className="p-4 border border-green-200  rounded-md">
                  <p className="font-medium mb-4">
                    Soubor byl úspěšně aktualizován! Nyní si můžete stáhnout výsledek:
                  </p>
          ...
          <Card className="mt-6">
            ...
            <CardBody>
              {advancedResult?.reportData && (
                <div className="mt-2 mb-6 border rounded-md p-4">
                  <h3 className="text-lg font-semibold mb-4">Výsledky pokročilého zpracování</h3>
                  ...
                  <div className="space-y-4">
                    ...
                    {advancedResult.reportData.nameComparisonReport && (
                      <div>
                        <h4 className="text-md font-semibold mb-2">Porovnání jmen:</h4>
                        <div 
                          className="p-3 mt-2 border rounded-md max-h-48 overflow-auto" 
                          dangerouslySetInnerHTML={{ __html: advancedResult.reportData.nameComparisonReport }}
                        />
                        <p className="text-sm text-gray-600 mt-2">
                          Jména byla porovnána mezi listy List1 a List2. V List1 byla opravena jména na základě jmen v List2. 
                          U opravených jmen je původní hodnota uložena ve sloupci A pro kontrolu.
                        </p>
                      </div>
                    )}
      ...
      <Card>
        ...
        <CardBody className="mt-0 p-4">
          <ul className="list-disc ml-6 mb-4">
            ...
            <li className="pt-1">Do listu "List2" budou přidána pouze nová data</li>
            <li className="pt-1">Formátování a vzorce listu "List1" budou zachovány</li>
      ...
      {uploadedFileName && (
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-8 mt-6">
          ...
          <div className="w-full flex justify-center mt-11">
            <Button
              type="submit"
              disabled={processState === ProcessState.LOADING || processState === ProcessState.ADVANCED_PROCESSING}
              color="primary"
              size="lg"
            >
