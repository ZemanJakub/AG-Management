...
export class NameComparisonService {
  ...
  public async compareAndFixNames(): Promise<NameComparisonOutput> {
    try {
      ...
      if (!list1 || !list2) {
        throw new Error('Některý z požadovaných listů (List1 nebo List2) neexistuje');
      }
      ...
      logger.info(`Načteno ${list2Names.length} jmen z List2`);
      ...
      for (let rowNum = this.options.list1StartRow; rowNum <= maxRow1; rowNum++) {
        ...
        logger.info(`Zpracovávám jméno v řádku ${rowNum}: ${originalName}`);
      ...
      logger.info(`Dokončeno porovnání jmen. Celkem: ${stats.total}, Přesných shod: ${stats.exactMatches}, Bezpečných shod: ${stats.safeMatches}, Nenalezených: ${stats.noMatches}`);
      
      // Kontrola konzistence statistik
      if (stats.exactMatches + stats.safeMatches + stats.noMatches !== stats.total) {
        logger.warn(`Nekonzistentní statistiky: součet typů shod (${stats.exactMatches + stats.safeMatches + stats.noMatches}) se nerovná celku (${stats.total})`);
      }
